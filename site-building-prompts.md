...
- The main navigation uses **Practice** as the merged destination for services and who we help.
...
- Added About, Practice, Our process, Results, FAQ, Contact, and mobile navigation.
- Added `provisa-record.jpg` as the primary visual.
- Added restrained entrance motion and a tactile paper grain treatment.

### Client update

- Added a dedicated **Our team** section with four placeholder portraits and team-desk descriptions.
- Added **Recognitions / proof of global relevance** as its own section.
...
- Added Blog and FAQ to the navbar.
...
- Added a WhatsApp support button using `https://wa.me/2348160550258`. Replace this number if the client supplies a different support number.
...
> Continue the Provisa Writers standalone Template 2 site from this file. Do not remove the Practice merge, the client contact links, the Our team, Recognitions, Blog, or FAQ sections. Treat the current admin credentials and local-storage blog as temporary prototype-only behavior. Before launch, move authentication, posts, image storage, and email delivery to secure server-backed services, and replace all placeholders with client-approved assets and copy. First compare the supplied previous-site link or screenshot with the current navigation and content, then make only the missing information architecture changes.